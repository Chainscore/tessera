...
def generate_package_rst(
    package_path: Path, display_name: str, doc_path: Path, force: bool = False
) -> None:
    ...
    content = [f"{display_name} package", "=" * (len(display_name) + 8), ""]

    if subpackages:
        content.extend(
            ["Subpackages", "-----------", "", ".. toctree::", "   :maxdepth: 2", ""]
        )
...
def generate_module_rst(
    module_path: Path, display_name: str, doc_path: Path, force: bool = False
) -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Sphinx documentation structure"
    )
    parser.add_argument(
    ...
    for root, dirs, files in os.walk(src_dir):
        ...
        if root_path == src_dir:
            ...
            pkg_display_name = "jam." + str(root_path.relative_to(src_dir)).replace(
                os.sep, "."
            )
        ...
        for file in files:
            if file.endswith(".py") and not file.startswith("_"):
                ...
                generate_module_rst(
                    file_path, module_display_name, doc_path, args.force
                )
