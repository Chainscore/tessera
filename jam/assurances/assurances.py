...
from jam.utils.constants import VALIDATOR_COUNT, SIGNING_CONTEXTS, UNAVAILABLE_WORK_EXPIRY
from jam.types.protocol.crypto import Ed25519Public, Ed25519Signature, OpaqueHash
...
class Assurances:
    ...
    def transition(state: State, block: Block) -> State:
        ...
        for assurance in assurances:
            ...
            if assurance.anchor != block.header.parent:
                raise AssurancesError(AssurancesErrorCode.BAD_ATTESTATION_PARENT, f"Assurance anchor {assurance.anchor} does not match block parent {block.header.parent}")

            # 3. Ensure the assurance signatures are valid
            Assurances.ensure_valid_signature(state.kappa[assurance.validator_index].ed25519, assurance.signature, assurance.bitfield, block.header.parent)

            # Update core assurances
            for i in range(len(assurance.bitfield)):
                if state.rho[i] == Null:
                    raise AssurancesError(AssurancesErrorCode.CORE_NOT_ENGAGED, f"Pending work report {i} not found")
        ...
        for i in range(len(state.rho)):
            if core_assurances[i] > super_majority or (block.header.slot >= state.rho[i].timeout + UNAVAILABLE_WORK_EXPIRY):
                new_state.rho[i] = OptionalWorkReportState(Null)

        return new_state

    @staticmethod
    def ensure_valid_signature(public_key: Ed25519Public, signature: Ed25519Signature, bitfield: AvailBitField, parent: OpaqueHash) -> None:
        """Ensure the signature is valid"""
        try:
            Ed25519PublicKey.from_public_bytes(bytes(public_key)).verify(
                bytes(signature), 
                SIGNING_CONTEXTS["available"] + bytes(Hash.blake2b(bytes(parent) + bitfield.encode()))
            )
        except InvalidSignature:
            raise AssurancesError(AssurancesErrorCode.BAD_SIGNATURE, f"Assurance signature {signature} is invalid")
    ...
    def ensure_validators_valid(assurances: List[AvailAssurance]) -> None:
        ...
        for assurance in assurances:
            if assurance.validator_index >= VALIDATOR_COUNT:
                raise AssurancesError(AssurancesErrorCode.BAD_VALIDATOR_INDEX, f"Validator index {assurance.validator_index} is invalid")
    ...
    def ensure_assurances_order(assurances: List[AvailAssurance]) -> None:
        ...
        if assurances != expected_assurances:
            raise AssurancesError(AssurancesErrorCode.NOT_SORTED_OR_UNIQUE_ASSURERS, "Assurances are not ordered by validator index")
    ...
    def ensure_assurances_unique(assurances: List[AvailAssurance]) -> None:
        ...
        if len(assurances) != len(set(assurance.validator_index for assurance in assurances)):
            raise AssurancesError(AssurancesErrorCode.NOT_SORTED_OR_UNIQUE_ASSURERS, "Assurances are not unique by validator index")
