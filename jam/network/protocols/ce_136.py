...
class WorkReportRequest(NetworkProtocol):
    ...
    async def transmit(self, data: CE136Data, assurers: Assurers = None) -> WorkReport | None:
        ...
        self.logger.info(f"Requesting Work-Report from {len(assurers) if assurers else 0} Validators")

        for client in node.all_connected:
            try:
                if assurers and client.validator_index not in assurers:
                    continue

                self.logger.trace("Requesting report from", peer=client)
                ...
                if res is not None:
                    return res
                        ...
                        self.logger.error("Error fetching work report from: ", peer=client)
    ...
    async def req_intercept(self, stream_id: int, server: PeerConnection):
        ...
        self.logger.trace("Received Work Report Request")
        try:
            ...
            self.logger.trace("Fetching Work Report")
    ...
    async def res_intercept(self, stream_id: int, client: PeerConnection) -> WorkReport | None:
