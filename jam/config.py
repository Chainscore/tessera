...
class NodeConfig(BaseModel):
    ...
    def __init__(self, _env_file: Optional[str] = None, **kwargs):
        ...
        env_values = {}
        target_fields = type(self).model_fields
        ...
        # Merge: JAM_ env vars > kwargs > .env values > defaults
        # Split env_values into JAM_ prefixed (explicit CLI) vs .env loaded
        final_values = {**kwargs}
        # JAM_ env vars override everything (explicit CLI intent)
        for field_name in target_fields:
            env_key = f"JAM_{field_name}"
            if env_key in os.environ:
                final_values[field_name] = env_values[field_name]
            elif field_name not in kwargs and field_name in env_values:
                final_values[field_name] = env_values[field_name]
