...
from jam.api.demo.models import (
...
app = FastAPI(
    ...
    openapi_tags=[
        {
            "name": "Types",
            "description": "Operations related to data types and type conversions",
        },
        {
            "name": "State Transitions",
            "description": "Validate state transitions for different components",
        },
...
@app.get(
    ...
    responses={200: {"description": "List of available types"}},
)
...
@app.post(
    "/api/v1/types/type_id/json_to_codec/validate",
...
async def json_to_codec(request_data: RequestDataCodec):
    ...
    try:
        ...
        return {
            "data": DATA_TYPES[label].from_json(request_data.input.file).encode().hex(),
            "status": "Ok",
        }
...
@app.post(
    "/api/v1/types/type_id/codec_to_json/validate",
    status_code=status.HTTP_200_OK,
    tags=["Types"],
...
@app.post(
    "/api/v1/recent_history/validate",
    tags=["State Transitions"],
    ...
    responses={
        200: {
            ...
            "model": Union[Dict[str, bool], ErrorResponse],
        }
    },
)
