...
class BandersnatchCurve:
    ...
    def elligator2_map(u: int, J: int, K: int, Z: int, p: int) -> Tuple[int, int]:
        ...
        if (1 + Z * pow(u, 2, p)) % p == 0:
            ...
            x1 = (
                -J
                * BandersnatchCurve.mod_inverse(K, p)
                * BandersnatchCurve.mod_inverse(1 + Z * pow(u, 2, p), p)
            ) % p
        ...
        gx1 = (
            pow(x1, 3, p)
            + (J * pow(x1, 2, p) * BandersnatchCurve.mod_inverse(K, p))
            + (x1 * BandersnatchCurve.mod_inverse(K * K, p))
        ) % p
        ...
        gx2 = (
            pow(x2, 3, p)
            + (J * pow(x2, 2, p) * BandersnatchCurve.mod_inverse(K, p))
            + (x2 * BandersnatchCurve.mod_inverse(K * K, p))
        ) % p
    ...
    def check_is_point(v: int, w: int) -> bool:
        ...
        lhs = (
            BandersnatchCurve.a * pow(v, 2, BandersnatchCurve.P)
            + pow(w, 2, BandersnatchCurve.P)
        ) % BandersnatchCurve.P
        rhs = (
            1
            + BandersnatchCurve.d
            * pow(v, 2, BandersnatchCurve.P)
            * pow(w, 2, BandersnatchCurve.P)
        ) % BandersnatchCurve.P
        return lhs == rhs
    ...
    def generate_curve_point(u: int) -> Tuple[int, int]:
        ...
        J, K = BandersnatchCurve.calculate_montgomery_params(
            BandersnatchCurve.a, BandersnatchCurve.d, BandersnatchCurve.P
        )
