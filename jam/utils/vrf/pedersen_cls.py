...
class PedersenVRF:
    ...
    def point_add(P1: Tuple[int, int], P2: Tuple[int, int]) -> Tuple[int, int]:
        ...
        try:
            x3 = (
                (x1y2 + y1x2) * PedersenVRF.mod_inverse(1 + dx1x2y1y2, PedersenVRF.p)
            ) % PedersenVRF.p
            y3 = (
                (y1 * y2 - PedersenVRF.a * x1 * x2)
                * PedersenVRF.mod_inverse(1 - dx1x2y1y2, PedersenVRF.p)
            ) % PedersenVRF.p
        except ValueError:
            return (0, 1)
    ...
    def point_double(P: Tuple[int, int]) -> Tuple[int, int]:
        ...
        x3 = (
            2 * x1 * y1 * PedersenVRF.mod_inverse(denom_x, PedersenVRF.p)
        ) % PedersenVRF.p
        y3 = (
            (y1**2 - PedersenVRF.a * x1**2)
            * PedersenVRF.mod_inverse(denom_y, PedersenVRF.p)
        ) % PedersenVRF.p
    ...
    def challenge(Y, I, O, R, Ok, ad) -> int:
        ...
        data = (
            PedersenVRF.point_to_string(Y)
            + PedersenVRF.point_to_string(I)
            + PedersenVRF.point_to_string(O)
            + PedersenVRF.point_to_string(R)
            + PedersenVRF.point_to_string(Ok)
            + ad.encode()
        )
    ...
    def generate_ticket(
        ...
        Y = PedersenVRF.point_add(
            PedersenVRF.point_mul(secret_key, PedersenVRF.G),
            PedersenVRF.point_mul(blinding_factor, PedersenVRF.B),
        )
        R = PedersenVRF.point_add(
            PedersenVRF.point_mul(k, PedersenVRF.G),
            PedersenVRF.point_mul(Kb, PedersenVRF.B),
        )
    ...
    def verify_ticket(input_point, ad, output_point, proof):
        ...
        Theta0 = PedersenVRF.point_add(
            Ok, PedersenVRF.point_mul(c, output_point)
        ) == PedersenVRF.point_mul(s, input_point)
        Theta1 = PedersenVRF.point_add(
            R, PedersenVRF.point_mul(c, Y)
        ) == PedersenVRF.point_add(
            PedersenVRF.point_mul(s, PedersenVRF.G),
            PedersenVRF.point_mul(Sb, PedersenVRF.G),
        )
...
def main():
    ...
    for i in range(len(validators)):
        ...
        input_point = BandersnatchCurve.generate_random_point(
            PedersenVRF.hash_to_point(validators[i])
        )
        ad = f"Ticket {i + 1}"

        output_point, proof = PedersenVRF.generate_ticket(
            secret_key, b, input_point, ad
        )
