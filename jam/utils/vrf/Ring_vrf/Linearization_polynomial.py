...
def linearization_polynomial(
    ...
    l2 = simplify(
        (zeta - omega ** (N - 4))
        * (b_zeta * (accx_poly - px_zeta) ** 2 * accx_poly + (1 - b_zeta) * accy_poly)
    )
    l3 = simplify(
        (zeta - omega ** (N - 4))
        * (
            (b_zeta * (accx_poly - px_zeta) + 1 - b_zeta) * accx_poly
            + b_zeta * (accx_poly - px_zeta) * accy_poly
        )
    )

    l_poly = simplify(
        alpha_coeffs[0] * l1 + alpha_coeffs[1] * l2 + alpha_coeffs[2] * l3
    )
...
def main():
    ...
    linearization_polynomial(
        zeta, omega, accip_poly, accx_poly, accy_poly, px_zeta, py_zeta, b_zeta, alpha_c
    )
