...
def interpolate(values, domain):
    ...
    polynomial = sum(
        padded_values[i] * prod((x - domain[j]) / (domain[i] - domain[j]) for j in range(N) if j != i)
        for i in range(N)
    )
...
def pcs_commit(polynomial, srs):
    commitment = sum(coef * srs[i] for i, coef in enumerate(polynomial.as_coefficients_dict().values()))
    return commitment % p
...
def conditional_addition_constraints(accx_poly, accy_poly, px_poly, py_poly, b_poly, omega, N):
    x = symbols('x')  # Symbol for the polynomial variable
...
def example_usage():
    ...
    print("c1x",c1x)
    print(' ')
    c2x,c3x=conditional_addition_constraints(accx_poly,accy_poly,px_poly,py_poly,b_poly,omega,N)
    print("c4x",c2x)
    print(' ')
    print("c3x",c3x)
    print(' ')
    ...
    R_point=R_expected_result_delta.compute_conditional_sum(b,public_keys,Seed_point,p)

    print("c4x",c4x)
    print(' ')
    c5x,c6x,c7x=boundary_constraints(accx_poly,accy_poly,accip_poly,Seed_point,R_point,D)
    print("c5x",c5x)
    print(' ')
    print("c6x",c6x)
    print(' ')
    ...
    cx=Fiat_shamir.construct_the_aggregated_polynomial(alpha_list,c1x,c2x,c3x,c4x,c5x,c6x,c7x)
    ...
    print(' ')
    px_zeta=Relevant_polynomial_Evoluation.evaluate_polynomials(px_poly,Evolution_point)
    py_zeta=Relevant_polynomial_Evoluation.evaluate_polynomials(py_poly,Evolution_point)
    s_zeta=Relevant_polynomial_Evoluation.evaluate_polynomials(s_poly,Evolution_point)
    b_zeta=Relevant_polynomial_Evoluation.evaluate_polynomials(b_poly,Evolution_point)
    accip_zeta=Relevant_polynomial_Evoluation.evaluate_polynomials(accip_poly,Evolution_point)
    accx_zeta=Relevant_polynomial_Evoluation.evaluate_polynomials(accx_poly,Evolution_point)
    accy_zeta=Relevant_polynomial_Evoluation.evaluate_polynomials(accy_poly,Evolution_point)
    ...
    l_zeta_omega, l_poly=Linearization_polynomial.linearization_polynomial(Evolution_point,omega,accip_poly,accx_poly,accy_poly,px_zeta,py_zeta,b_zeta,alpha_list[:3],N,p)
    print(" ")
    print(l_zeta_omega,l_poly)

    v_list=Fiat_shamir.fiat_shamir_hash(px_zeta,py_zeta,s_zeta,b_zeta,accip_zeta,accx_zeta,accy_zeta,l_zeta_omega)
    v_chunks=Fiat_shamir.split_hash_into_chunks(v_list,8)

    print("v_chunks:",v_chunks)
    print()

    agg_x=Fiat_shamir.construct_the_agg_x(v_chunks,px_poly,py_poly,s_poly,b_poly,accip_poly,accx_poly,accy_poly,q_poly)
    print("agg_x:",agg_x)
    print()
    ...
    phi=[Cb,Caccip,Caccx,Caccy,px_poly,py_poly,s_zeta,b_zeta,accip_zeta,accx_zeta,accy_zeta,cq,l_zeta_omega,phi_zeta,phi_zeta_omega]
