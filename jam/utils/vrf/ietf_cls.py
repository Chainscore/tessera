...
class IETF:
    ...
    def point_add(self, P1: Tuple[int, int], P2: Tuple[int, int]) -> Tuple[int, int]:
        ...
        try:
            ...
            y3 = (
                (y1y2 - self.a * x1x2) * self.mod_inverse(1 - dx1x2y1y2, self.p)
            ) % self.p
        except ValueError:
            return (0, 1)
    ...
    def challenge(
        ...
        data = (
            ...
            + self.point_to_string(I)
            + self.point_to_string(O)
    ...
    def generate_ticket(
        self, secret_key: int, input_point: Tuple[int, int], ad: str
    ...
    def verify_ticket(
        ...
        """Verify a VRF ticket"""
        c, s = proof
        U = self.point_subtract(
            self.point_mul(s, self.G), self.point_mul(c, public_key)
        )
        V = self.point_subtract(
            self.point_mul(s, input_point), self.point_mul(c, output_point)
        )
        return c == self.challenge(
    ...
    def run_example(self):
        ...
        for i in range(len(validators)):
            ...
            input_point = BandersnatchCurve.generate_random_point(
                self.hash_to_point(validators[i])
            )
            ...
            tickets.append(
                {"proof": proof, "valid": valid, "output_point": output_point}
            )
