...
class DictionaryCodec(Codec[Mapping[K, V]], Generic[K, V]):
    ...
    def encode_size(self, value: Mapping[Codable[K], Codable[V]]) -> int:
        ...
        for k, v in pairs:
            try:
                ...
                raise EncodeError(0, 0, f"Expected Codable, got {type(k).__name__} or {type(v).__name__}")
    ...
    def encode_into(self, value: Mapping[Codable[K], Codable[V]], buffer: bytearray, offset: int = 0) -> int:
        """
        ...
        try:
            ...
            for key, val in pairs:
                try:
                    ...
                    raise EncodeError(0, 0, f"Expected Codable, got {type(key).__name__} or {type(val).__name__}")
    ...
    def decode_from(
        ...
        try:
            ...
            for _ in range(length):
                ...
                # Decode value
                value, value_size = value_codable_class.decode_from(buffer, current_offset)
