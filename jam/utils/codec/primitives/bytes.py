...
class FixedBytesCodec(Codec):
    ...
    @staticmethod
    def decode_from(buffer: Union[bytes, bytearray, memoryview], offset: int = 0, size: int = 0) -> Tuple[bytes, int]:
        """Decode a byte sequence from a buffer."""
        if len(buffer) - offset < size:
            raise ValueError(f"Buffer too small. Expected {size} bytes, got {len(buffer) - offset}")
