        ...
        try:
            ...
            if target_type in (str, int, float, bool):
                if not isinstance(data, target_type):
                    raise JsonDeserializationError(f"Expected {target_type}, got {type(data)}")
                return data
            ...
            if target_type is dict:
                ...
                return target_type({
                    JsonCodec.from_json(k, key_type): JsonCodec.from_json(v, value_type)
            ...
            try:
                ...
                raise JsonDeserializationError(f"Cannot deserialize to type {target_type}. Full error: {e}")
            
        except (TypeError, ValueError) as e:
            raise JsonDeserializationError(f"Error deserializing to {target_type}: {str(e)}")
            ...
            if json_name not in data:
                ...
                try:
                    if hasattr(info['type'], 'from_json'):
                        field_values[field_name] = info['type'].from_json(data[json_name])
                    else:
                    ...
                    if not isinstance(field_values[field_name], info['type']):
                        raise JsonFieldError(field_name, info['type'], f"Expected {info['type']}, got {type(field_values[field_name])}")
                except Exception as e:
                    raise JsonFieldError(field_name, info['type'], str(e))
