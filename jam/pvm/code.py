...
class Code(Codable):
    ...
    def execute(self, gas: Gas, pc = ProgramCounter(0)):
        memory=Memory.from_pc(self.read, self.r_write, self.args, self.z, self.s)
        registers=Registers.from_pc(self.args)
        
        # status, pc, remaining_gas, registers, memory
        return PVM.execute(blob=self.code, program_counter=pc, gas=gas, registers=registers, memory=memory)
        
    @classmethod
    def decode_from(cls, pc: bytes):
        offset = 0
