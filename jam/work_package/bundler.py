...
class Bundler:
    ...
    async def fetch_imports(self, w: WorkItem):
        ...
        for s_root in import_map:
            ...
            logger.info(f"Fetching segments..", seg_root=s_root.hex(), er_root=e_root.hex(), indices=seg_indices)
            ...
            for n in seg_indices:
                try:
                    ...
                    if n in segs_dict:
                        logger.debug("Cache Hit", seg_root=s_root.hex()[:16], er_root=e_root.hex()[:16], seg_ind=n)
                        fetched_imports[(s_root,n)] = segs_dict[n]

                    # If cache miss, check in DB, and cache it
                    else:
                        logger.debug("Cache Miss", seg_root=s_root.hex()[:16], er_root=e_root.hex()[:16], seg_ind=n)
                        logger.info("Fetching segments from root", segment_root=s_root.hex())
                    ...
                    logger.warning("Looking for segment shards in DA!", err=str(SEG_MISS), err_type=type(SEG_MISS).__name__)

                    try:
                        logger.debug("Segment Miss", seg_root=s_root.hex()[:16], er_root=e_root.hex()[:16], seg_ind=n)
                        ...
                        if not ss_dict or len(ss_dict) == 0:
                            logger.debug("Unrecognised Erasure root found!", er_root=e_root.hex()[:16])
                            raise KeyError("Unrecognised Erasure root found. No segment shards available.")
                        ...
                        if total_segments <= n:
                            raise IndexError(f"Total segments known for this er-root are {total_segments}, must be {total_count}")

                        if len(ss_dict) < chain_config.recovery_threshold:
                            logger.debug("Shards in database are less than recovery threshold", er_root=e_root.hex()[:16], shards=len(ss_dict))
                            raise KeyError("Not enough segment shards available in database!")
                        ...
                        for i in ind_to_req:
                            ...
                            if len(shards) >= chain_config.recovery_threshold:
                                ...
                                raise KeyError(f"Not enough shards available in database for segment {i}")
                        ...
                        for i in range(chain_config.num_validators):
                            ...
                            if self.node.validator_index != vi:
                                ...
                                if data and len(data) == 1 and data[0] and len(data[0]) == len(ind_to_req):
                                    responses = data[0]

                                    for s, s_i in zip(responses, ind_to_req):
                                        if s and isinstance(s, SegmentShard):
                                            if s_i in shards_dict:
                                                shards_dict[s_i].append((s, ShardIndex(i)))
                                            else:
                        ...
                        for i in ind_to_req:
                            ...
                            if len(shards) > chain_config.recovery_threshold:
                                ...
                                raise KeyError(f"Not enough shards fetched for segment {i}")
    ...
    async def fetch_justifications(self, w: WorkItem) -> Justifications:
        ...
        # Pre-computed map for imports
        # Segment Root -> ([(Segment Index to import, Its Relative proof page)], Its OG Work Report)
        import_map: Dict[SegmentRoot, Tuple[List[Tuple[SegmentIndex, SegmentIndex]], WorkReport]] = {}
        ...
        for s_root in import_map:
            ...
            logger.info(f"Fetching justifications..", seg_root=s_root.hex(), er_root=e_root.hex(), indices=proof_indices)
            ...
            for (n, p) in proof_indices:
                try:
                    ...
                    if p in proofs_dict:
                        logger.debug("Cache Hit", seg_root=s_root.hex()[:16], er_root=e_root.hex()[:16], seg_ind=n, proof_ind=(exports_count+p))
                        proof = proofs_dict[p]
                    ...
                    # If cache miss, check in DB, and cache it
                        ...
                        logger.debug("Cache Miss", seg_root=s_root.hex()[:16], er_root=e_root.hex()[:16], seg_ind=n, proof_ind=(exports_count+p))
                        logger.info("Fetching proof segments from root", segment_root=s_root.hex())
                        _, proofs = seg_da.get(s_root)

                        for i, proof in enumerate(proofs):
                            proofs_dict[i] = proof

                        logger.debug("Fetched proof segments from root", segment_root=s_root.hex(), count=len(proofs))
                        _, jfn = utils.zero_depth_proof(proofs[p], n)
                        fetched_jfns[(s_root, n)] = Justification(jfn)

                except KeyError as PROOF_MISS:
                    logger.warning("Looking for proof shards in DA!", err=str(PROOF_MISS), err_type=type(PROOF_MISS).__name__)

                    try:
                        logger.debug("Proof Miss", seg_root=s_root.hex()[:16], er_root=e_root.hex()[:16], seg_ind=n, proof_ind=(exports_count+p))
                        ...
                        if not ss_dict or len(ss_dict) == 0:
                            logger.debug("Unrecognised Erasure root found!", er_root=e_root.hex()[:16])
                            raise KeyError("Unrecognised Erasure root found. No proof shards available.")
                        ...
                        if len(ss_dict) < chain_config.recovery_threshold:
                            logger.debug("Shards in database are less than recovery threshold", er_root=e_root.hex()[:16], shards=len(ss_dict))
                            raise KeyError("Not enough proof shards available in database!")
                        ...
                        for i in ind_to_req:
                            ...
                            if len(shards) >= chain_config.recovery_threshold:
                                ...
                                raise KeyError(f"Not enough shards available in database for proof segment {i}")
                        ...
                        logger.debug(
                            ...
                            err=str(SHARD_MISS),
                            err_type=type(SHARD_MISS).__name__,
                            seg_root=s_root.hex()[:16], er_root=e_root.hex()[:16], seg_ind=n,
                                    ...
                                    for s, p_i in zip(responses, ind_to_req):
                                        if s and isinstance(s, SegmentShard):
                                            if p_i in shards_dict:
                                                shards_dict[p_i].append((s, ShardIndex(i)))
                                            else:
                        ...
                        for i in ind_to_req:
                            ...
                            if len(shards) > chain_config.recovery_threshold:
                                ...
                                raise KeyError(f"Not enough shards fetched for proof segment {i}")
        ...
        for spec in w.import_segments:
            try:
                ...
                logger.debug(f"Compiling justification.", s_root=s_root.hex()[:16], index=n)

                if (s_root, n) not in fetched_jfns:
                    raise KeyError(f"Proofs not found for root: {s_root.hex()[:16]}, index: {n}")
