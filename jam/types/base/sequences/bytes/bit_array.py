...
class BitArray(Array):
    ...
    def __init__(self, value: Bytable):
        ...
        if isinstance(value, BitArray) or (
            isinstance(value, Sequence)
            and all((isinstance(bit, (Bit, Boolean, bool))) for bit in value)
        ):
        ...
        data: list[bool] = ByteUtils.bytes_to_bitarray(
            ByteUtils.to_bytes(value),
            bitorder=self._bit_order,
...
def decodable_bit_array(
    length: int, bitorder: Literal["msb", "lsb"] | None = "msb"
) -> Callable[[Type[BitArray]], Type[BitArray]]:
    """
