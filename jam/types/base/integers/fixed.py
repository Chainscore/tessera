...
class FixedInt(BaseInteger, Codable):
    ...
    def _validate(self, value: int) -> None:
        ...
        if self.byte_size > 0:  # Fixed width
            min_value = 0 if not self.has_sign else -(1 << (8 * math.floor(self.byte_size / 2) - 1))
            max_value = (1 << (8 * self.byte_size)) - 1 if not self.has_sign else (1 << (8 * math.floor(self.byte_size / 2) - 1)) - 1
            if not min_value <= value <= max_value:
                raise ValueError(f"Value must be between {min_value} and {max_value}, got {value}")

def decodable_int(byte_size: int, has_sign = False) -> Callable[[Type[FixedInt]], Type[FixedInt]]:
