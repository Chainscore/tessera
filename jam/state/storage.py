...
class StateStorage:
    ...
    def save_n_clear_cache(
        self, hh: HeaderHash | None = None, kv: RockStore | None = None
    ):
        """
        ...
        if kv and hh:
            kv.put(self.get_storage_key(hh), _state_cache.encode())
    ...
    def get(
        self, key_bytes: bytes, fill_cache: bool = True, skip_cache=False
    ) -> bytes | None:
