...
class AuditingAndJudgement:
    ...
    def verifiable_random_selection(
        ...
        shuffle_not_null: List[Tuple[CoreIndex, WorkReport]] = [
            (c, w) for (c, w) in updated_array if w is not Null
        ][:3]
    ...
    def tranche_index(header_slot: TimeSlot) -> U8:
        ...
        tranche_index = (
            CURRENT_TIME() - (SLOT_PERIOD * int(header_slot))
        ) // AUDIT_PERIOD
    ...
    async def audit_refine(
        ...
        for i in range(VALIDATOR_COUNT):
            if i != s_i:
                ...
                print(
                    "REQUESTED SHARD_INDEX WITH NODE_INDEX ==>>",
                    request_s_i,
                    request_v_i,
                )
                ...
                data = await CE138.transmit(
                    node=node, data=data, node_index=ValidatorIndex(1)
                )
