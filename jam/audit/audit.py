...
#         vrf = IETF_VRF(Bandersnatch_TE_Curve, BandersnatchPoint)
#         entropy = vrf.proof_to_hash(BandersnatchPoint.encode_to_curve(AuditingAndJudgement.verifiable_random_quality(entropy=header)))
#
...
#
#         """
#         tranches =  (state.tau - (SLOT_PERIOD * header.slot)) // AUDIT_PERIOD
#         return tranches
#
...
class AuditingAndJudgement:
    ...
    def verifiable_random_quality(header: Header):
        ...
        bandersnatch_proof = bytes(vrf.proof_to_hash(BandersnatchPoint.encode_to_curve(bytes(entropy_yield_vrf_signature)))[:96])
    ...
    def non_empty_item(self, header: Header, state: Sigma):
        ...
        # Equation : 17.7 (r = y(So))
        vrf = IETF_VRF(Bandersnatch_TE_Curve, BandersnatchPoint)
        entropy = bytes(vrf.proof_to_hash(BandersnatchPoint.encode_to_curve(AuditingAndJudgement.verifiable_random_quality(header=header)))[:96])

        # Equation :  17.6 p = f([(c, Qc) | c <- Nc ], r )
        audit_report = self.report_to_be_audit(state.rho)
        core_report =  [(c, w_r) for c, w_r in enumerate(audit_report)]
    ...
    @staticmethod
    def current_tranches( header : Header) -> int:
        """
        Eq. 17.8
