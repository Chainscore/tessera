...
from jam.storage.tranche_store import Tranche, TrancheState, JudgmentRecord, tranche_store
...
class TrancheEngine:
    ...
    async def run(self, header_hash: HeaderHash):
        ...
        while True:
            ...
            logger.info(
                f"⚙️ Running tranche {tranche_index} for header {header_hash}, auditing {len(ts.unaudited_list)} WRs, valid {len(ts.valid_set)}, invalid {len(ts.invalid_set)}"
            )
            ...
            if tranche_index > 0:
                ...
                for wr in ts.unaudited_list:
                    ...
                    logger.debug(
                        f"WR {wr.hex()[:8]} | True: {true_count} | False: {false_count} | Accnouncements: {len(record.announces)} | Tranche: {tranche_index}"
                    )
                    # TODO: Will change this true count condition which is !mentioned in GP
                    if (
                        false_count == 0
                        and true_count >= VALIDATOR_COUNT * 2 // 3
                        or len(record.announces) == len(record.true_votes)
                    ):
                ...
                ts.unaudited_list = new_unaudited
                ...
                if not new_unaudited:
                    ...
                    logger.info(
                        f"✅ Block audited successfully for header {header_hash} after tranche {tranche_index}"
                    )
                    break

                logger.info(
                    f"🔄 Moving to tranche {tranche_index} for remaining {len(new_unaudited)} WRs"
                )
