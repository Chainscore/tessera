...
class TrancheEngine:
    ...
    async def run(self, slot_index: Uint[32]):
        ...
        """
        tranche_index = Uint[32](0)

        while True:
            # from jam.operations.ext_store import ext_store

            initTranche = Tranche(
                slot_index=Uint[32](slot_index), tranche_index=tranche_index
            )

            ts: TrancheState = self.store.load(initTranche)
            logger.info(
            ...
            if tranche_index > 0:
                ...
                for wr in ts.unaudited_list:
                    record: JudgmentRecord = ts.judgments.get(
                        wr.encode(), JudgmentRecord.empty()
                    )

                    true_count = len(record.true_votes)
                    false_count = len(record.false_votes)
                    logger.debug(
                        f"WR {wr.encode().hex()[:8]} | True: {true_count} | False: {false_count} | Tranche: {tranche_index}"
                    )
                ...
                if not new_unaudited:
                    ...
                    logger.info(
                        f"✅ Block audited successfully for slot {slot_index} after tranche {tranche_index}"
                    )
                    break
            ...
            tranche_index += Uint[32](1)

            updated_tranche = Tranche(
                slot_index=Uint[32](slot_index), tranche_index=tranche_index
            )
