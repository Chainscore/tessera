...
class AuditProcess:
    ...
    async def audit_process(cls, newly_avail_wrs: List[Option[WorkReport]]):
        ...
        final_list = sample_work_reports_with_nulls(
        ...
        try:
            ...
            reports = audit.verifiable_random_selection(
                entropy_source=entropy_,
                bandersnatch_key=node.b_key,
                pre_audit_report=p_a_r,
            )
    ...
    async def audit_announcement(cls, assign_wrs: List[Tuple[CoreIndex, WorkReport]]):
        ...
        announcement_sign = audit.validator_announcement_statement(
            assign_report=assign_wrs, header=header_hash, tranche=U8(0)
        )
        ...
        signature = audit.vrf_signature_bandersnatch(
    ...
    async def judgment_process(cls, assign_wrs: List[Tuple[CoreIndex, WorkReport]]):
        ...
        logger.info(
            f"Reports are avialable for judgment on this node is {len(assign_wrs)} "
        )
        ...
        try:
            for c, r in assign_wrs:
                ...
                package, core, extrinsic = get_work_package_by_rep_hash(
                    filepath="jam/combine.json", rep_hash=wr_hash
                )

                # print("=================================", "package =>", package, "core =>", core, "extrinsic =>", extrinsic)

                result = await audit.audit_refine(
                    p=package,
                    c=core,
                    e=extrinsic,
                    wr=r,
                    node_index=node.validator_index,
                )
