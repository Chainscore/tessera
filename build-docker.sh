...
echo "Building Docker image ${IMAGE_NAME}:${IMAGE_TAG} using Dockerfile.fuzz..."
docker build -f Dockerfile.fuzz -t "${IMAGE_NAME}:${IMAGE_TAG}" .
...
echo "  - default entrypoint: normal multi-node behavior"
