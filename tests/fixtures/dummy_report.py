...
from tests.fixtures.utils import create_dummy_Bytes, create_dummy_bytes32, create_dummy_bytes
...
def create_dummy_report() -> WorkReport:
    ...
    segment_lookup = SegmentRootLookupDict({OpaqueHash(create_dummy_bytes32()): OpaqueHash(create_dummy_bytes32())})
    temp_segment_lookup = SegmentRootLookupDict({})
    return WorkReport(
        package_spec= specs,
        context= create_dummy_work_context(),
        core_index= U16(1),
        authorizer_hash=OpaqueHash(create_dummy_bytes32()),
        auth_output=create_dummy_Bytes(2),
        auth_gas_used= Gas(10),
        segment_root_lookup= temp_segment_lookup,
