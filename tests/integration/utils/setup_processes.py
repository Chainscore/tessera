...
from ..genesis.ghost_node import run_node_process
...
class Client:
    ...
    genesis: bool = field(default=True)


async def setup_processes(
    clients: list[Client], node_task: Optional[Callable], max_time=20
):
    processes = []

    for client in clients:
        if client.role == Role.PJAM:
            p = Process(target=run_polkajam, args=("", client.idx))
        else:
