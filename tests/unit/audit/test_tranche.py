...
def audit_condition(db,work_report:WorkReport,tranche_index:int):
    """
    refer 17.19
    """

    if (len(J_f(db,wr=work_report,tranch=tranche_index)) and len(An(db,work_report,tranche=tranche_index)) == len(J_t(db,work_report,tranche=tranche_index))) || len(J_t(db,work_report,tranche=tranche_index))== 2/3*VALIDATOR_COUNT):
        return True
    else:
        return False
