...
def test_h2f():
    ...
    def process_field_elements(big_ints: list[BigInt]) -> list[int]:
        """
        Convert a list of BigInt instances (with inner limbs in little-endian order)
        to Python integers.

        Each BigInt.inner list is assumed to contain 64-bit limbs, where the 0th element  
