...
from tsrkit_types import Vector, ByteArray, TypedVector, structure, Bytes
...
from jam.utils.merkle.binary_merkle import ChoicedHashes, BMRFunctions, OpaqueHashes
...
def trace_fn(
    values: TypedVector[Bytes], index: int, hash_fn=Hash.blake2b
) -> ChoicedHashes:
...
def test_paged_proof():
    ...
    for x in range(page_count):
        # path = merklizer.merkle_path_fn(values=segments, size=int(6), index=int(x))
        values = segments
        size = int(6)
        index = int(x)
    ...
    single_proofs = TypedVector[ChoicedHashes]([])
    for i in range(6):
        single_proof = merklizer.subtree_path(segments, int(0), int(i))
