...
def test_slot_regression_error():
    ...
    initial_state = create_state(
        ...
        gamma_s=GammaS(
            GammaSFallback(
                [keys.bandersnatch for keys in create_validator_data_from_keys() * 2]
            )
        ),
        gamma_z=GammaZ(
    ...
    with pytest.raises(SafroleError) as excinfo:
        Safrole.transition(
            initial_state, regression_block, entropy=Bytes[32](create_dummy_bytes(32))
        )
...
def test_invalid_seal_signature():
    ...
    # with pytest.raises(SafroleError) as excinfo:
    Safrole.transition(
        initial_state, invalid_seal_block, Bytes[32](create_dummy_bytes(32))
    )
    # TODO: Uncomment this once the signatures are implemented
