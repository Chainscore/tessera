...
def test_ticket_accumulation():
    ...
    ticket1 = TicketBody(
        id=TicketId(vrf_output(ticket_envelope1.signature)),
        attempt=ticket_envelope1.attempt
    )
    ticket2 = TicketBody(
        id=TicketId(vrf_output(ticket_envelope2.signature)),
        attempt=ticket_envelope2.attempt
    ) 
    ...
    new_block = create_block(
        ...
        tickets=[ticket_envelope2, ticket_envelope1]
    )
    
    # Apply the transition
    new_state = Safrole.transition(initial_state, initial_state, new_block, Bytes[32](create_dummy_bytes(32)))
...
def test_ticket_submission_outside_period():
    ...
    with pytest.raises(SafroleError) as excinfo:
        Safrole.transition(initial_state, initial_state, new_block, Bytes[32](create_dummy_bytes(32)))
