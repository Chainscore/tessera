...
def test_basic_wp_building(db_path):
    ...
    state.delta[package.auth_code_host].service = AccountMetadata(
        code_hash=code_hash,
    ...
    state.delta[package.auth_code_host].lookup[
        LookupTable(hash=code_hash, length=BlobLength(len(service_code)))
    ...
    print(
        f"🎉 Work Item executed | Status: {r} | Gas consumed {u} | Exported Segments {e}"
    )
