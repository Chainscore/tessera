...
from .data import create_test_state, create_test_block, create_valid_judgement_votes, deepcopy
...
class TestDisputesValidationErrors:
    ...
    def test_invalid_signature(self):
        ...
        disputes_extrinsic = DisputesExtrinsic(
            verdicts=Verdicts([good_verdict]), culprits=Culprits([]), faults=Faults([])
        )
    ...
    def test_invalid_judgment_age(self):
        ...
        disputes_extrinsic = DisputesExtrinsic(
            verdicts=Verdicts([old_verdict]), culprits=Culprits([]), faults=Faults([])
        )
    ...
    def test_invalid_vote_split(self):
        ...
        if VALIDATORS_WONKY < 5:  # JudgementVotes has minimum length of 5
            pytest.skip(
                f"VALIDATORS_WONKY ({VALIDATORS_WONKY}) is too small for minimum JudgementVotes length"
            )

        # Skip if we can't create an invalid vote count
        if VALIDATORS_WONKY - 1 < 5:
            pytest.skip(
                f"Cannot create invalid vote count with VALIDATORS_WONKY={VALIDATORS_WONKY}"
            )
        ...
        invalid_verdict = Verdict(
            target=target_hash,
            age=U32(0),
            votes=create_valid_judgement_votes(
                target_hash, True, VALIDATORS_WONKY - 1
            ),  # Not enough for wonky
        )

        disputes_extrinsic = DisputesExtrinsic(
            verdicts=Verdicts([invalid_verdict]), culprits=Culprits([]), faults=Faults([])
        )
