...
from jam.types.protocol.crypto import (
    Ed25519Public, Ed25519Signature, WorkReportHash, 
    BandersnatchPublic, BlsPublic
)
...
def create_valid_judgement_votes(target_hash: bytes, vote_value: bool, count: int) -> JudgementVotes:
    ...
    for i in range(count):
        ...
        votes.append(Judgement(
            ...
            signature=Ed25519Signature(Ed25519PrivateKey.from_private_bytes(keys.ed25519_private).sign((X.VALID if vote_value else X.INVALID).value + target_hash))
        ))
...
def create_sorted_faults(target: WorkReportHash, vote: bool, keys: list[Ed25519Public]) -> list[Fault]:
...
def create_sorted_verdicts(verdicts_data: list[tuple[WorkReportHash, U32, bool, int]]) -> list[Verdict]:
...
def assert_state_counts(initial_counts: dict, new_state: Sigma, 
                       good_delta: int = 0, bad_delta: int = 0, 
                       wonky_delta: int = 0, offenders_delta: int = 0):
...
def assert_targets_in_sets(new_state: Sigma, 
