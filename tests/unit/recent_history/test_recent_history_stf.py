...
def create_block_from_input(input: Input) -> Block:
    ...
    block.accumulation_root = input.accumulate_root
...
def create_state_from_pre(pre_state: PreState) -> State:
    ...
    state.beta = pre_state.beta
    return state


def vector_transition(vector: Testcase) -> Boolean:
    test_state = create_state_from_pre(().pre_state)
    test_block = create_block_from_input(().input)
    try:
        output = RecentHistory.transition(test_state, test_block)
        assert output.beta == ().post_state.beta
